# 🏗️ Backend Architecture

백엔드 시스템 아키텍처 및 설계 문서입니다.

---

## 📐 시스템 아키텍처 다이어그램

```
┌─────────────────────────────────────────────────────┐
│                       CLIENT LAYER                  │
├─────────────────┬─────────────────┬─────────────────┤
│   Web Browser   │  Samsung App    │   Apple App     │
│   (React)       │  (React Native) │   (Swift)       │ └────────┬────────┴────────┬────────┴────────┬────────┘
         │                 │                 │
         │    HTTP/REST    │                 │
         ▼                 ▼                 ▼
┌──────────────────────────────────────────────────────────────────────────┐
│                           FASTAPI APPLICATION                            │
│                                                                          │
│  ┌────────────────────────────────────────────────────────────────────┐  │
│  │                         API LAYER (Routers)                        │  │
│  │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌────────────┐ │  │
│  │  │file_upload_  │ │auto_upload_  │ │  chat_api    │ │ user_api   │ │  │
│  │  │api.py        │ │api.py        │ │              │ │            │ │  │
│  │  └──────┬───────┘ └──────┬───────┘ └──────┬───────┘ └─────┬──────┘ │  │
│  │         │                │                │               │        │  │
│  │  ┌──────────────┐ ┌──────────────┐                        │        │  │
│  │  │similar_api   │ │vectordb_     │                        │        │  │
│  │  │              │ │router        │                        │        │  │
│  │  └──────┬───────┘ └──────┬───────┘                        │        │  │
│  └─────────┼────────────────┼────────────────────────────────┼────────┘  │
│            │                │                                │           │
│            ▼                ▼                                ▼           │
│  ┌────────────────────────────────────────────────────────────────────┐  │
│  │                       SERVICE LAYER                                 │  │
│  │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌────────────┐ │  │
│  │  │file_upload_  │ │auto_upload_  │ │chat_service  │ │similar_    │ │  │
│  │  │service.py    │ │service.py    │ │              │ │service.py  │ │  │
│  │  └──────┬───────┘ └──────┬───────┘ └──────┬───────┘ └─────┬──────┘ │  │
│  └─────────┼────────────────┼────────────────┼───────────────┼────────┘  │
│            │                │                │               │           │
│            ▼                ▼                ▼               ▼           │
│  ┌────────────────────────────────────────────────────────────────────┐  │
│  │                         CORE LAYER                                 │  │
│  │                                                                    │  │
│  │  ┌─────────────────────────────────────────────────────────────┐   │  │
│  │  │                    CHATBOT ENGINE                           │   │  │
│  │  │  ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌──────────┐  │   │  │
│  │  │  │chat_       │ │fixed_      │ │intent_     │ │persona   │  │   │  │
│  │  │  │generator   │ │responses   │ │classifier  │ │          │  │   │  │
│  │  │  └────────────┘ └────────────┘ └────────────┘ └──────────┘  │   │  │
│  │  │  ┌────────────┐                                             │   │  │
│  │  │  │rag_query   │                                             │   │  │
│  │  │  └────────────┘                                             │   │  │
│  │  └─────────────────────────────────────────────────────────────┘   │  │
│  │                                                                    │  │
│  │  ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌────────────────┐   │  │
│  │  │health_     │ │llm_        │ │vector_     │ │rag_query       │   │  │
│  │  │interpreter │ │analysis    │ │store       │ │(core)          │   │  │
│  │  └────────────┘ └────────────┘ └────────────┘ └────────────────┘   │  │
│  │                                                                    │  │
│  │  ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌────────────────┐   │  │
│  │  │db_parser   │ │db_to_json  │ │unzipper    │ │adaptive_       │   │  │
│  │  │            │ │            │ │            │ │threshold       │   │  │
│  │  └────────────┘ └────────────┘ └────────────┘ └────────────────┘   │  │
│  └────────────────────────────────────────────────────────────────────┘  │
│                                    │                                     │
│                                    ▼                                     │
│  ┌────────────────────────────────────────────────────────────────────┐  │
│  │                        UTILS LAYER                                 │  │
│  │  ┌────────────┐ ┌────────────────────┐ ┌───────────────────────┐   │  │
│  │  │preprocess  │ │preprocess_for_     │ │platform_detection     │   │  │
│  │  │            │ │embedding           │ │                       │   │  │
│  │  └────────────┘ └────────────────────┘ └───────────────────────┘   │  │
│  └────────────────────────────────────────────────────────────────────┘  │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────┐
│                          EXTERNAL SERVICES                               │
│  ┌────────────────────┐           ┌────────────────────────────────────┐ │
│  │     ChromaDB       │           │         OpenAI API                 │ │
│  │  (Vector Store)    │           │  ┌────────────┐ ┌────────────────┐ │ │
│  │                    │           │  │Embeddings  │ │Chat Completions│ │ │
│  │  ./chroma_data/    │           │  │(text-emb-  │ │(gpt-4o-mini)   │ │ │
│  │                    │           │  │3-small)    │ │                │ │ │
│  └────────────────────┘           │  └────────────┘ └────────────────┘ │ │
│                                   └────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────────┘
```

---

## 🔷 레이어 구조

### Layer 1: API Layer (Routers)

**역할**: HTTP 요청 수신, 입력 검증, 응답 반환

```
app/api/
├── file_upload_api.py   # ZIP/DB 파일 업로드
├── auto_upload_api.py   # 앱 JSON 업로드
├── chat_api.py          # 챗봇 (자유형/고정형)
├── user_api.py          # 사용자 분석
└── similar_api.py       # 유사도 검색
```

**특징**:

- Pydantic 모델로 입력 검증
- FastAPI Router로 엔드포인트 분리
- 비즈니스 로직은 Service 레이어에 위임

### Layer 2: Service Layer

**역할**: 비즈니스 로직 조합, 트랜잭션 관리

```
app/service/
├── file_upload_service.py   # ZIP 처리 전체 흐름
├── auto_upload_service.py   # API 업로드 전체 흐름
├── chat_service.py          # 챗봇 서비스
└── similar_service.py       # 유사도 검색 서비스
```

**특징**:

- Core 모듈들을 조합
- 비동기 처리 (ThreadPoolExecutor)
- 에러 핸들링 및 로깅

### Layer 3: Core Layer

**역할**: 핵심 비즈니스 로직, 알고리즘

```
app/core/
├── health_interpreter.py   # 규칙 기반 건강 해석
├── llm_analysis.py         # LLM 분석 엔진
├── vector_store.py         # VectorDB 관리
├── rag_query.py            # RAG 쿼리 빌더
├── db_parser.py            # Samsung DB 파서
├── db_to_json.py           # SQLite 변환
├── unzipper.py             # ZIP 해제
├── adaptive_threshold.py   # 적응형 임계값
│
└── chatbot_engine/         # 챗봇 전용 모듈
    ├── chat_generator.py   # 자유형 응답 생성
    ├── fixed_responses.py  # 고정형 응답 생성
    ├── intent_classifier.py # 의도 분류
    ├── persona.py          # 캐릭터 페르소나
    └── rag_query.py        # 챗봇용 RAG
```

### Layer 4: Utils Layer

**역할**: 공통 유틸리티, 데이터 변환

```
app/utils/
├── preprocess.py              # 건강 데이터 전처리
├── preprocess_for_embedding.py # 임베딩용 텍스트 생성
└── platform_detection.py      # 플랫폼 감지
```

---

## 📡 API 엔드포인트 맵

### 데이터 업로드

| 엔드포인트         | 메서드 | Input                                                  | Output                                        |
| ------------------ | ------ | ------------------------------------------------------ | --------------------------------------------- |
| `/api/file/upload` | POST   | multipart (file, user_id, difficulty, duration)        | { summary, llm_result, total_days_saved }     |
| `/api/auto/upload` | POST   | JSON { user_id, date, raw_json, difficulty, duration } | { summary, analysis, ai_recommended_routine } |

### 분석

| 엔드포인트                  | 메서드 | Input                                   | Output                                        |
| --------------------------- | ------ | --------------------------------------- | --------------------------------------------- |
| `/api/user/latest-analysis` | GET    | Query { user_id, difficulty, duration } | { summary, analysis, ai_recommended_routine } |
| `/api/user/raw-history`     | GET    | Query { user_id }                       | { data: [...] }                               |

### 챗봇

| 엔드포인트        | 메서드 | Input                                      | Output                  |
| ----------------- | ------ | ------------------------------------------ | ----------------------- |
| `/api/chat`       | POST   | JSON { user_id, message, character }       | { character, response } |
| `/api/chat/fixed` | POST   | JSON { user_id, question_type, character } | { character, response } |

### VectorDB 관리

| 엔드포인트                     | 메서드 | Input            | Output                 |
| ------------------------------ | ------ | ---------------- | ---------------------- |
| `/api/vectordb/status`         | GET    | None             | { total_count, users } |
| `/api/vectordb/user/{user_id}` | GET    | Path { user_id } | { count, data }        |

### 유사도 검색

| 엔드포인트     | 메서드 | Input                     | Output      |
| -------------- | ------ | ------------------------- | ----------- |
| `/api/similar` | POST   | JSON { summary, user_id } | { results } |

---

## 🔄 서비스 간 상호작용

### 1. 데이터 업로드 플로우

```
[ZIP Upload Flow]

file_upload_api.py
       │
       ▼
file_upload_service.py
       │
       ├──────────────┬──────────────┬──────────────┐
       ▼              ▼              ▼              ▼
  unzipper.py   db_to_json.py   db_parser.py   preprocess.py
       │              │              │              │
       └──────────────┴──────────────┴──────────────┘
                             │
                             ▼
                    vector_store.py ◄────► ChromaDB
                             │
                             ▼
                    llm_analysis.py ◄────► OpenAI API
                             │
                             ▼
                health_interpreter.py
```

### 2. 분석 플로우

```
[Analysis Flow]

user_api.py
       │
       ▼
vector_store.py ◄────► ChromaDB
       │
       ▼
llm_analysis.py
       │
       ├──────────────────────────────────────────────┐
       ▼                                              ▼
health_interpreter.py                         vector_store.py
       │                                      (RAG 검색)
       │                                              │
       ├──────────────────────────────────────────────┘
       │
       ├─── [Fallback 조건] ──► get_fallback_routine()
       │
       └─── [LLM 호출] ──► OpenAI API ──► validate_routine()
```

### 3. 챗봇 플로우

```
[자유형 Chat Flow]

chat_api.py
       │
       ▼
chat_service.py → handle_chat()
       │
       ▼
chat_generator.py → generate()
       │
       ├─────────────────────────────────────────┐
       ▼                                         ▼
intent_classifier.py                     persona.py
       │                                         │
       ▼                                         │
  ┌────┴────┐                                    │
  │         │                                    │
  ▼         ▼                                    │
health   routine                                 │
query    request                                 │
  │         │                                    │
  ▼         ▼                                    │
rag_query.py                                     │
  │                                              │
  ▼                                              │
vector_store.py ◄────► ChromaDB                  │
  │                                              │
  └──────────────────────┬───────────────────────┘
                         │
                         ▼
                   OpenAI API
                   (응답 생성)
```

```
[고정형 Chat Flow]

chat_api.py
       │
       ▼
chat_service.py → handle_fixed_chat()
       │
       ▼
fixed_responses.py → generate_fixed_response()
       │
       ├─────────────────────────────────────────┐
       ▼                                         ▼
vector_store.py                          persona.py
(최근 5일 조회)                                  │
       │                                         │
       ▼                                         │
health_interpreter.py                            │
       │                                         │
       ▼                                         │
  ┌────┴────────────────────────────────────┐    │
  │ question_type별 분기                    │    │
  ├─ weekly_report    → LLM 호출           │    │
  ├─ today_recommendation → run_llm_analysis│    │
  ├─ weekly_steps     → LLM 호출           │    │
  ├─ sleep_report     → LLM 호출           │    │
  ├─ heart_rate       → LLM 호출           │    │
  └─ health_score     → LLM 호출           │    │
       │                                         │
       └─────────────────────┬───────────────────┘
                             │
                             ▼
                    캐릭터 템플릿 적용
                             │
                             ▼
                        응답 반환
```

---

## 🗄️ 데이터 저장소

### ChromaDB (Vector Store)

```
./chroma_data/
└── health_summaries/     # Collection
    ├── embeddings        # OpenAI text-embedding-3-small
    ├── documents         # Natural language summaries
    └── metadatas         # user_id, date, source, health_score, ...
```

**Document ID 형식**:

```python
doc_id = f"{user_id}_{date}_{source}"
# 예: "user@email.com_2025-12-17_api_samsung"
```

**Metadata 구조**:

```python
{
    "user_id": "user@email.com",
    "date": "2025-12-17",
    "timestamp": 20251217,
    "health_score": 75,
    "recommended_intensity": "중",
    "source": "api_samsung",
    "platform": "samsung",
    "updated_at": "20251217143000",
    "summary_json": "{...}"
}
```

---

## ⚙️ 설정 관리

### config.py

```python
# LLM 설정
LLM_MODEL_MAIN = os.getenv("LLM_MODEL", "gpt-4o-mini")
LLM_TEMPERATURE = float(os.getenv("LLM_TEMPERATURE", "0.3"))
LLM_MAX_TOKENS = int(os.getenv("LLM_MAX_TOKENS", "2048"))

# 기본값
DEFAULT_DIFFICULTY = "중"
DEFAULT_DURATION = 30

# ChromaDB
CHROMA_PERSIST_DIR = "./chroma_data"
CHROMA_COLLECTION_NAME = "health_summaries"

# CORS
ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", "http://localhost:3000").split(",")
```

### .env (예시)

```env
OPENAI_API_KEY=sk-xxx
LLM_MODEL=gpt-4o-mini
LLM_TEMPERATURE=0.3
LLM_MAX_TOKENS=2048
ALLOWED_ORIGINS=http://localhost:3000,http://192.168.0.15:3000
```

---

## 🚀 서버 실행

```bash
# 개발 모드
uvicorn main:app --reload --host 0.0.0.0 --port 8000

# 프로덕션 모드
uvicorn main:app --host 0.0.0.0 --port 8000 --workers 4
```

---

## 📊 의존성 구조

```
main.py
├── config.py
├── app/api/
│   ├── file_upload_api.py ──► file_upload_service.py
│   ├── auto_upload_api.py ──► auto_upload_service.py
│   ├── chat_api.py ──► chat_service.py
│   ├── user_api.py ──► vector_store.py, llm_analysis.py
│   └── similar_api.py ──► similar_service.py
│
├── app/service/
│   ├── file_upload_service.py ──► unzipper, db_to_json, db_parser,
│   │                              preprocess, vector_store, llm_analysis
│   ├── auto_upload_service.py ──► preprocess, vector_store, llm_analysis
│   ├── chat_service.py ──► chat_generator, fixed_responses
│   └── similar_service.py ──► vector_store
│
├── app/core/
│   ├── health_interpreter.py (독립)
│   ├── llm_analysis.py ──► health_interpreter, vector_store, rag_query
│   ├── vector_store.py ──► chromadb, openai
│   ├── rag_query.py (독립)
│   ├── db_parser.py (독립)
│   ├── db_to_json.py (독립)
│   ├── unzipper.py (독립)
│   └── chatbot_engine/
│       ├── chat_generator.py ──► intent_classifier, persona, rag_query
│       ├── fixed_responses.py ──► persona, vector_store, llm_analysis
│       ├── intent_classifier.py (독립)
│       ├── persona.py (독립)
│       └── rag_query.py ──► vector_store
│
└── app/utils/
    ├── preprocess.py (독립)
    ├── preprocess_for_embedding.py (독립)
    └── platform_detection.py (독립)
```
